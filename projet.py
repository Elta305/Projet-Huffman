...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.sidebar_button_1 = customtkinter.CTkButton(
            self.sidebar_frame, text="Charger fichier", command=self.load_file)
        self.sidebar_button_1.grid(row=1, column=0, padx=20, pady=10)
        self.sidebar_button_2 = customtkinter.CTkButton(
            self.sidebar_frame, text="Charger clé", command=self.load_key)
        self.sidebar_button_2.grid(row=2, column=0, padx=20, pady=10)
        self.sidebar_button_3 = customtkinter.CTkButton(
            self.sidebar_frame, text="Charger fichier encodé", command=self.load_encoded_file)
        ...
        self.main_button_1 = customtkinter.CTkButton(master=self, fg_color="transparent", border_width=2, text_color=(
            "gray10", "#DCE4EE"), text="Encoder", command=self.encode)
        ...
        self.main_button_2 = customtkinter.CTkButton(master=self, fg_color="transparent", border_width=2, text_color=(
            "gray10", "#DCE4EE"), text="Décoder", command=self.decode)
        self.main_button_2.grid(row=1, column=3, padx=(
            20, 20), pady=(20, 20), sticky="nsew")
        self.main_button_3 = customtkinter.CTkButton(master=self, fg_color="transparent", border_width=2, text_color=(
            "gray10", "#DCE4EE"), text="Afficher Arbre", command=self.open_toplevel)
        self.main_button_3.grid(row=2, column=3, padx=(
            20, 20), pady=(20, 20), sticky="nsew")

        self.textbox_1 = customtkinter.CTkTextbox(self, width=250, height=50)
        self.textbox_1.grid(row=0, column=1, columnspan=2,
        ...
        self.textbox_2.insert("0.0", texte2 := {'r': '000', ' ': '001', 'e': '01', 'T': '1000',
                              'x': '1001', 't': '1010', 'à': '1011', 'n': '1100', 'c': '1101', 'o': '1110', 'd': '1111'})
        self.textbox_3.insert(
            "0.0", texte3 := "1000011001101001001101100101110011011110111101000")
    ...
    def load_encoded_file(self):
        ...
        if len(texte_encode) > 1000:
            self.textbox_3.insert("0.0", texte_encode[:1000] + " ... (Le code est trop long pour être affiché en entier)")
        else:
    ...
    def encode(self):
        if self.textbox_1.get("0.0", "end")[:][:-1] != self.fichier.fichier:
            self.fichier.fichier = self.textbox_1.get("0.0", "end")[:][:-1]
    ...
    def decode(self):
        ...
        self.textbox_1.delete("0.0", "end")
        if len(texte_decode) > 1000:
            self.textbox_1.insert("0.0", texte_decode[:1000] + " ... (Le texte est trop long pour être affiché en entier)")
        else:
    ...
    def open_toplevel(self):
        if self.toplevel_window is None or not self.toplevel_window.winfo_exists():
            ...
            self.toplevel_window = a.ToplevelWindow(
                self.fichier.creation_arbre()[0])
            self.toplevel_window.afficher_arbre()
        else:
