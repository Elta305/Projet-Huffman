...
class Huffman():
    ...
    def creation_arbre(self, dict_char):
        """
        Crée un arbre de Huffman à partir du dictionnaire comptant le nombre
        d'occurrences de chaque caractère.
        """
    ...
    def code_binaire(self):
        """
        Appelle la fonction récursive _code_binaire pour modifier
        l'attribut code_binaire de chaque sommet de l'arbre.
        """
    ...
    def _code_binaire(self, sommet, code):
        """
        Modifie récurssivement l'attribut code_binaire de chaque sommet
        """
...
def decodage_char(dict, char):
    """ dict, str -> str
        Retourne un string avec le caractère char décodé mais sans le nombre d'itération.
    """

    if char in dict.values():
        return list(dict.keys())[list(dict.values()).index(char)]
    return None